package com.sanyedu.feedback.imageloader;
...
import com.sanyedu.feedback.app.FeedbackApplication;
...
public class PicassoLoader implements ILoaderStrategy {
	...
	private static LruCache sLruCache = new LruCache(FeedbackApplication.gApp);

	private static Picasso getPicasso() {
		if (sPicassoSingleton == null) {
			synchronized (PicassoLoader.class) {
				if (sPicassoSingleton == null) {
					sPicassoSingleton = new Picasso.Builder(FeedbackApplication.gApp).memoryCache(sLruCache).build();
				}
			}
		}
	...
	public void clearDiskCache() {
		File diskFile = new File(FeedbackApplication.gApp.getCacheDir(), PICASSO_CACHE);
		if (diskFile.exists()) {
			//这边自行写删除代码
