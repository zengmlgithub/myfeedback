...
//                .addParams(HttpUtil.UpdateObj.TYPE, type)
    ...
    @Override
    public void ModifyEmail(String id, final String email) {

        if(!CheckUtils.isParasLegality(id,email)){
            SanyLogs.e("params is null ,return");
            return;
        }

        SanyLogs.i("email:" + email);
        String url = HttpUtil.getPort(HttpUtil.UPDATE_PERSON_OBJ_PORT);
        OkHttpUtils
                .post()
                .url(url)
//                .addParams(HttpUtil.UpdateObj.TYPE, type)
                .addParams(HttpUtil.UpdateObj.TE_ID,id)
                .addParams(HttpUtil.UpdateObj.TE_EMAIL,email)
                .build()
                .execute(
                        new BaseModelCallback<String>(){
